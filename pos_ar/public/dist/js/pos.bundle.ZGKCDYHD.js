(()=>{var D=Object.defineProperty;var w=Object.getOwnPropertySymbols;var O=Object.prototype.hasOwnProperty,L=Object.prototype.propertyIsEnumerable;var C=(t,e,s)=>e in t?D(t,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):t[e]=s,y=(t,e)=>{for(var s in e||(e={}))O.call(e,s)&&C(t,s,e[s]);if(w)for(var s of w(e))L.call(e,s)&&C(t,s,e[s]);return t};frappe.provide("pos_ar.myaccessories");pos_ar.myaccessories.AccessoriesController=class{constructor(t){this.wrapper=$(t).find(".layout-main-section"),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/accessories_page/main.css">'),this.selectedCompany=frappe.defaults.get_user_default("company"),this.selectedPOSOpening="",this.selectedBrand="",this.brandList=[],this.make()}make(){this.createLayout()}createLayout(){let t=$('<div class="accessories-container">').appendTo(this.wrapper),e=$('<div class="top-bar shadow">').appendTo(t),s=$('<div class="top-bar-left">').appendTo(e);$('<div class="page-icon"><i class="fa fa-box-open fa-lg"></i></div>').appendTo(s),$("<h2>").text("Accessories").appendTo(s);let i=$('<div class="top-bar-center">').appendTo(e),n=$('<div class="filter-container">').appendTo(i),a=$('<div class="filter-group">').appendTo(n);this.companySelect=$("<select>").addClass("form-control").change(()=>{this.selectedCompany=this.companySelect.val(),this.loadPOSOpenings(),this.loadItems(t.find(".items-container"))}).appendTo(a);let o=$('<div class="filter-group">').appendTo(n);this.posOpeningSelect=$("<select>").addClass("form-control").change(()=>{this.selectedPOSOpening=this.posOpeningSelect.val(),this.loadItems(t.find(".items-container"))}).appendTo(o);let r=$('<div class="filter-group">').appendTo(n),c=$('<div class="brand-input-wrapper">').appendTo(r);this.brandField=frappe.ui.form.make_control({parent:c,df:{fieldtype:"Link",options:"Brand",placeholder:"Type to search brands...",only_select:!1,filter_fields:["name"],get_query:()=>({filters:{}})},render_input:!0}),this.brandField.refresh(),this.brandField.$input.addClass("brand-filter-input").removeClass("input-with-feedback");let l=$("<button>").addClass("clear-brand-btn").html('<i class="fa fa-times"></i>').click(()=>{this.brandField.set_value(""),this.selectedBrand="",this.loadItems(t.find(".items-container"))}).appendTo(c);this.brandField.$input.on("change",()=>{let m=this.brandField.get_value();this.selectedBrand!==m&&(this.selectedBrand=m,console.log("Brand changed to:",this.selectedBrand),setTimeout(()=>{this.loadItems(t.find(".items-container"))},100))}),this.brandField.$input.on("awesomplete-selectcomplete",()=>{let m=this.brandField.get_value();this.selectedBrand!==m&&(this.selectedBrand=m,console.log("Brand selected from dropdown:",this.selectedBrand),this.loadItems(t.find(".items-container")))}),frappe.call({method:"frappe.client.get_list",args:{doctype:"Company",fields:["name"],limit:0},callback:m=>{m.message&&(this.companySelect.empty(),m.message.forEach(_=>{this.companySelect.append($("<option></option>").val(_.name).text(_.name))}),this.companySelect.val(this.selectedCompany),this.loadPOSOpenings())}});let d=$('<div class="top-bar-right">').appendTo(e);$('<button class="btn btn-primary btn-export">').html('<i class="fa fa-download mr-2"></i>Export').click(()=>this.exportData()).appendTo(d);let h=$('<div class="items-container">').appendTo(t),p=$('<div class="item-row header">').html(`
            <div class="item-col name">Name</div>
            <div class="item-col qty">Quantity</div>
            <div class="item-col total">Total</div>
        `).appendTo(h);this.loadItems(h)}formatCurrency(t){return t.toFixed(2)+" DA"}loadPOSOpenings(){frappe.call({method:"frappe.client.get_list",args:{doctype:"POS Opening Entry",filters:{company:this.selectedCompany,docstatus:1},fields:["name","pos_profile"],limit:0},callback:t=>{t.message&&(this.posOpeningSelect.empty(),this.posOpeningSelect.append($("<option></option>").val("").text("All POS Sessions")),t.message.forEach(e=>{this.posOpeningSelect.append($("<option></option>").val(e.name).text(`${e.name} (${e.pos_profile})`))}),this.posOpeningSelect.val(this.selectedPOSOpening))}})}loadItems(t){t.find(".item-row:not(.header)").remove(),t.find(".loading-spinner, .error-message, .no-items-message").remove();let e=$(`
            <div class="loading-spinner">
                <div class="spinner"></div>
                <div class="loading-text">Loading items...</div>
            </div>
        `).appendTo(t);console.log("Loading items with filters:",{company:this.selectedCompany,pos_opening_entry:this.selectedPOSOpening,brand:this.selectedBrand}),frappe.call({method:"pos_ar.pos_ar.doctype.pos_info.pos_info.get_saled_item",args:{company:this.selectedCompany,pos_opening_entry:this.selectedPOSOpening,brand:this.selectedBrand||""},callback:s=>{e.remove(),console.log("Response:",s),s.message&&s.message.items&&Object.keys(s.message.items).length>0?(this.data=s.message.items,this.renderItems(t,this.data)):$('<div class="no-items-message">').text("No items found").appendTo(t)},error:s=>{e.remove(),$('<div class="error-message">').text("Error loading items. Please try again.").appendTo(t),console.error("Error loading items:",s)}})}renderItems(t,e){if(t.find(".item-row:not(.header)").remove(),Object.keys(e).length===0){$('<div class="item-row no-data">').html('<div class="item-col name">No sales data found for selected date</div>').appendTo(t);return}let s=0;Object.entries(e).forEach(([i,n])=>{s+=n.rate*n.qty,$('<div class="item-row">').html(`
                    <div class="item-col name">${frappe.utils.escape_html(i)}</div>
                    <div class="item-col qty">${n.qty}</div>
                    <div class="item-col total">${this.formatCurrency(n.rate*n.qty)}</div>
                `).appendTo(t)}),$('<div class="item-row grand-total">').html(`
                <div class="item-col name">Grand Total</div>
                <div class="item-col qty"></div>
                <div class="item-col total">${this.formatCurrency(s)}</div>
            `).appendTo(t)}exportData(){let t=Object.entries(this.data||{}).map(([h,p])=>({name:h,qty:p.qty,total:p.rate*p.qty}));if(t.length===0){frappe.msgprint("No data to export.");return}let e=t.length,s=t.reduce((h,p)=>h+(p.qty||0),0),i=t.reduce((h,p)=>h+(p.total||0),0);t.push({name:"Total",qty:s,total:i});let n=XLSX.utils.book_new(),a=XLSX.utils.json_to_sheet(t),o=t.length;a[`A${o+1}`]={t:"s",v:"Total Records"},a[`B${o+1}`]={t:"n",v:e},XLSX.utils.book_append_sheet(n,a,"Sheet1");let r=XLSX.write(n,{bookType:"xlsx",type:"array"}),c=new Blob([r],{type:"application/octet-stream"}),l=URL.createObjectURL(c),d=document.createElement("a");d.href=l,d.download="data.xlsx",document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(l)}};frappe.provide("pos_ar.Pricing");pos_ar.Pricing.PricingController=class{constructor(t){this.wrapper=$(t).find(".layout-main-section"),this.current_screen="pricing",this.fetcher=new pos_ar.Pricing.PricingFetcher,this.setup_page(),this.add_style(),this.setup_events(),this.load_companies(),this.priceLists=[],this.brands=[],this.priceMap={}}setup_page(){this.page_content=$(`
        ...
        `).appendTo(this.wrapper),this.loadingPopover=document.getElementById("loadingPopover")}setup_events(){this.wrapper.find(".nav-item").on("click",t=>{let e=$(t.currentTarget).data("screen");this.switch_screen(e)}),this.wrapper.find(".company-filter").on("change",t=>{let e=$(t.currentTarget).val();this.filter_by_company(e)}),this.wrapper.find(".btn-primary").on("click",()=>{this.create_new_item_price()}),this.wrapper.find('.btn-default:contains("Export")').on("click",()=>{this.export_pricing_data()}),this.wrapper.on("click",".fix-prices",t=>{let e=$(t.currentTarget),s=e.data("brand"),i=e.data("price-list"),n=this.priceMap[`${s}_${i}`],a=[...new Set(n.map(r=>r.price))],o=new frappe.ui.Dialog({title:__(`Fix Prices for ${s}`),fields:[{fieldtype:"HTML",fieldname:"current_prices",label:__("Current Prices"),options:`
            ...
            `);return}let n=JSON.parse(localStorage.getItem("priceListOrder")||"[]");n.length>0?e.sort((r,c)=>{let l=n.indexOf(r.name),d=n.indexOf(c.name);return l===-1&&d===-1?0:l===-1?1:d===-1?-1:l-d}):localStorage.setItem("priceListOrder",JSON.stringify(e.map(r=>r.name))),this.priceMap={},t.forEach(r=>{let c=`${r.brand||"No Brand"}_${r.price_list}`;this.priceMap[c]||(this.priceMap[c]=[]),this.priceMap[c].push({name:r.name,price:r.price_list_rate,item_code:r.item_code})});let a=$(`
                    ...
                    <table class="table table-bordered table-hover">
                        <thead>
                            <tr class="headers">
                                ...
                                ${e.map(r=>`
                                    <th class="sortable draggable-header" data-sort="price" data-price-list="${r.name}" draggable="true">
                                        <div class="header-content">
                                            <i class="fa fa-grip-vertical drag-handle"></i>
                                            ${r.name} <i class="fa fa-sort"></i>
                                        </div>
                                    </th>
                                `).join("")}
                            </tr>
                        ...
                        <tbody>
                            ${s.map(r=>`
                                <tr>
                                    <td>${r.brand||r.name}</td>
                                    ${e.map(c=>{let l=this.priceMap[`${r.name}_${c.name}`]||[],d=l.length>1&&!l.every(_=>_.price===l[0].price),h=e.map(_=>{let u=this.priceMap[`${r.name}_${_.name}`]||[];return u.length>0?u[0].price:0}).filter(_=>_>0),p=l.length>0?l[0].price:0,m="";if(p>0&&h.length>0){let _=Math.max(...h),u=Math.min(...h),g=(_-u)/6;p>=_-g?m="price-highest":p>=_-2*g?m="price-high":p>=_-3*g?m="price-medium-high":p>=_-4*g?m="price-medium-low":p>=_-5*g?m="price-low":m="price-lowest"}return`
                                            <td>
                                                ${l.length>0?`<div class="price-cell ${d?"different-prices":""} ${m}">
                                                        <div class="price-value">
                                                            ${frappe.format(l[0].price,{fieldtype:"Currency"})}
        ...
        `);i.append(o).append(a)}async load_fixing_data(t){if(!!t)try{this.loadingPopover||(this.loadingPopover=document.getElementById("loadingPopover")),this.loadingPopover.matches(":popover-open")&&this.loadingPopover.hidePopover(),requestAnimationFrame(()=>{this.loadingPopover.showPopover()});let e=await this.fetcher.fetchAllItemPrices(t),s=e.prices;this.priceLists=e.price_lists,this.brands=e.brands,this.loadingPopover.hidePopover(),this.render_Fixing_data(s,this.priceLists,this.brands)}catch(e){this.loadingPopover&&this.loadingPopover.hidePopover(),frappe.msgprint({title:__("Error"),indicator:"red",message:__("Failed to load item prices")}),console.error("Error loading item prices:",e)}}render_Fixing_data(t,e,s){let i=this.wrapper.find(".fixing-screen");if(i.empty(),!t||t.length===0){i.html(`
                <div class="no-data-state">
                    <div class="no-data-icon">
                        <i class="fa fa-tag"></i>
                    </div>
        ...
        `);i.append(o).append(a)}setupTableEvents(t){let e=this;t.find(".toggle-filters").on("click",function(){t.find(".filters").toggle()}),t.find(".clear-filters").on("click",function(){t.find(".filters input, .global-search").val(""),t.find("tbody tr").show()}),t.find(".global-search").on("input",function(){let a=$(this).val().toLowerCase();t.find("tbody tr").each(function(){let o=$(this),r=o.text().toLowerCase();o.toggle(r.includes(a))})}),t.find(".brand-filter, .price-filter").on("input",function(){let o=t.find(".filters input").map(function(){return{column:$(this).closest("td").index(),value:$(this).val().toLowerCase()}}).get();t.find("tbody tr").each(function(){let r=$(this),c=o.every(l=>{let d=r.find(`td:eq(${l.column})`).text().toLowerCase();return!l.value||d.includes(l.value)});r.toggle(c)})});let s={column:null,direction:"asc"};t.find(".sortable").on("click",function(){let a=$(this).data("sort"),o=$(this).data("price-list"),r=$(this).index();s.column===r?s.direction=s.direction==="asc"?"desc":"asc":s={column:r,direction:"asc"},t.find(".sortable i").attr("class","fa fa-sort"),$(this).find("i").attr("class",`fa fa-sort-${s.direction}`);let c=t.find("tbody tr").get();c.sort((l,d)=>{let h=$(l).find(`td:eq(${r})`).text(),p=$(d).find(`td:eq(${r})`).text();return a==="price"&&(h=parseFloat(h.replace(/[^0-9.-]+/g,""))||0,p=parseFloat(p.replace(/[^0-9.-]+/g,""))||0),s.direction==="asc"?h>p?1:-1:h<p?1:-1}),t.find("tbody").html(c)}),t.on("click",".set-brand-prices",a=>{let o=$(a.currentTarget).data("brand");frappe.confirm(`This will set prices for all items of brand "${o}". Do you want to continue?`,()=>{frappe.call({method:"pos_ar.pos_ar.page.pricing.pricing.add_price_for_all_item_by_brand2",args:{brand:o},freeze:!0,freeze_message:__("Setting Prices..."),callback:r=>{if(!r.exc){frappe.show_alert({message:__("Prices set successfully"),indicator:"green"});let c=$(".company-filter").val();this.current_screen==="fixing"?this.load_fixing_data(c):this.load_pricing_data(c)}}})})}),t.on("click",".edit-price",a=>{a.stopPropagation();let o=$(a.currentTarget).data("name");this.show_price_editor(o)});let i=t.find(".draggable-header"),n=null;i.each((a,o)=>{$(o).on("dragstart",r=>{n=o,$(o).addClass("dragging"),r.originalEvent.dataTransfer.setData("text/plain",$(o).data("priceList"))}),$(o).on("dragend",()=>{$(n).removeClass("dragging"),n=null}),$(o).on("dragover",r=>{r.preventDefault()}),$(o).on("drop",r=>{if(r.preventDefault(),!n||n===o)return;let c=t.find(".draggable-header"),l=c.index(n),d=c.index(o);t.find("tr").each((g,x)=>{let v=$(x).find("td, th"),b=v.eq(l+1),I=v.eq(d+1),k=b.html(),P=I.html(),S={sort:b.data("sort"),priceList:b.data("priceList")},B={sort:I.data("sort"),priceList:I.data("priceList")};b.html(P),I.html(k),$(x).hasClass("headers")&&(b.data(B),I.data(S))});let p=JSON.parse(localStorage.getItem("priceListOrder")||"[]"),m=$(n).data("priceList"),_=$(o).data("priceList"),u=p.filter(g=>g!==m&&g!==_),f=Math.min(p.indexOf(_),p.indexOf(m));u.splice(f,0,_,m),localStorage.setItem("priceListOrder",JSON.stringify(u))})})}create_new_item_price(){let t=this.wrapper.find(".company-filter").val();if(!t){frappe.throw(__("Please select a company first"));return}frappe.prompt([{label:"Brand",fieldname:"brand",fieldtype:"Link",options:"Brand",reqd:1},{label:"Price List",fieldname:"price_list",fieldtype:"Link",options:"Price List",reqd:1},{label:"Price List Rate",fieldname:"price_list_rate",fieldtype:"Currency",reqd:1}],e=>{frappe.db.get_list("Item",{filters:{brand:e.brand}}).then(s=>{if(s.length>0){let i=s[0].name;frappe.call({method:"frappe.client.insert",args:{doc:{doctype:"Item Price",item_code:i,price_list:e.price_list,price_list_rate:e.price_list_rate,company:t}},callback:n=>{n.message&&(frappe.show_alert({message:__("Item Price created successfully"),indicator:"green"}),this.load_pricing_data(t))}})}else frappe.msgprint({title:__("Error"),indicator:"red",message:__("No item with the specified brand exists. Please create an item with the specified brand first.")})})},"Create New Item Price","Create")}show_price_editor(t){frappe.db.get_doc("Item Price",t).then(e=>{frappe.prompt([{label:"Current Price",fieldname:"current_price",fieldtype:"Currency",read_only:1,default:e.price_list_rate},{label:"New Price",fieldname:"new_price",fieldtype:"Currency",reqd:1}],s=>{frappe.call({method:"pos_ar.pos_ar.page.pricing.pricing.fix_prices",args:{brand:e.brand,price_list:e.price_list,new_price:s.new_price},freeze:!0,freeze_message:__("Updating Price..."),callback:i=>{if(i.exc){frappe.msgprint({title:__("Error"),indicator:"red",message:__("Failed to update price")});return}frappe.show_alert({message:__("Price updated successfully"),indicator:"green"});let n=$(`.edit-price[data-name="${t}"]`);if(n.length){let a=n.closest(".price-cell");if(a.length){let o=frappe.format(s.new_price,{fieldtype:"Currency"});a.html(`
                                        <div class="price-value">
                                            ${o}
                                            <button class="btn btn-xs btn-default edit-price" 
                                                data-name="${t}"
                                    ...
                                    `)}}}})},__("Update Price"),__("Update"))}).catch(e=>{console.error(e),frappe.msgprint({title:__("Error"),indicator:"red",message:__("Failed to fetch price details")})})}export_pricing_data(){let t=this.wrapper.find(".company-filter").val();if(!t){frappe.msgprint({title:__("Error"),indicator:"red",message:__("Please select a company first")});return}let e="data:text/csv;charset=utf-8,";e+="Brand,"+this.priceLists.map(n=>n.name).join(",")+`
`,console.log("the price lists : ",e),this.brands.forEach(n=>{let a=n.brand;this.priceLists.forEach(o=>{let r=this.priceMap[`${n.name}_${o.name}`]||[];a+=","+(r.length>0?r.map(c=>c.price).join(","):"empty")}),e+=a+`
`});let s=encodeURI(e),i=document.createElement("a");i.setAttribute("href",s),i.setAttribute("download",`pricing_matrix_${t}.csv`),document.body.appendChild(i),i.click()}add_style(){this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/pricing_page/main.css">');let t=`
        ...
        `;this.wrapper.append(t)}};pos_ar.Pricing.PricingFetcher=class{constructor(){console.log("PricingFetcher initialized")}async fetchItemPrices(t){try{let e=await frappe.call({method:"pos_ar.pos_ar.doctype.pos_info.pos_info.get_item_prices",args:{company:t}});return console.log("fetched item prices",e.message),e.message||[]}catch(e){return console.error("Error fetching item prices:",e),[]}}async fetchAllItemPrices(t){try{let e=await frappe.call({method:"pos_ar.pos_ar.page.pricing.pricing.get_all_item_prices",args:{company:t}});return console.log("fetched item prices",e.message),e.message||[]}catch(e){return console.error("Error fetching item prices:",e),[]}}};pos_ar.PointOfSale.Controller=class{constructor(t){this.wrapper=$(t).find(".layout-main-section"),this.page=t.page,this.selectedItemMaps=new Map,this.selectedItem={name:""},this.selectedField={field_name:""},this.selectedTab={tabName:""},this.selectedPaymentMethod={methodName:""},this.defaultCustomer={name:"",customer_name:""},this.defaultPriceList={name:""},this.taxes_and_charges_template=null,this.taxes_and_charges=[],this.payment_methods=[],this.POSOpeningEntry={},this.invoiceData={netTotal:0,grandTotal:0,paidAmount:0,toChange:0,discount:0},this.db=null,this.syncInput=!1,this.start_app()}async start_app(){try{this.db=await pos_ar.PointOfSale.pos_db.openDatabase(),this.settings_data=new pos_ar.PointOfSale.posSettingsData(this.db),this.dataHandler=new pos_ar.PointOfSale.FetchHandler,this.appData=new pos_ar.PointOfSale.posAppData(this.db,this.dataHandler),await this.appData.getAllData(),this.screenManager=new pos_ar.PointOfSale.ScreenManager(this.settings_data),this.toggleKeyboardMode(!this.settings_data.settings.showItemDetails),this.prepare_container(),await this.prepare_app_data(),await this.checkForPOSEntry(),await this.prepare_components(),this.checkUnSyncedPos(),this.setListeners();let t=await this.appData.getAndDeleteAllOpenedPosInvoice();this.restorePosInvoices(t)}catch(t){console.error("halfware POS Err ==> ",t)}}async prepare_app_data(){try{await this.handleAppData();let t=this.getCustomerDefaultPriceList(this.defaultCustomer.name);(t==""||t==null||t==null)&&(t=this.defaultPriceList.name);let e=frappe.model.get_new_doc("POS Invoice");e.customer=this.defaultCustomer.name,e.pos_profile=this.appData.appData.pos_profile.name,e.items=[],e.taxes_and_charges=this.appData.appData.pos_profile.taxes_and_charges,e.additional_discount_percentage=0,e.paid_amount=0,e.base_paid_amount=0,e.creation_time=frappe.datetime.now_datetime(),e.payments=this.getPaymentMethods(),e.is_pos=1,e.update_stock=1,e.docstatus=0,e.status="Draft",e.priceList=t,e.opened=1;let s=new Date,[i,n,a]=s.toISOString().split("T")[0].split("-"),o=s.getHours(),r=s.getMinutes(),c=s.getMilliseconds();e.refNum=this.appData.appData.pos_profile.name+"-"+i+"-"+n+"-"+a+"-"+o+r+c,e.custom_cach_name=e.refNum,this.selectedItemMaps.set("C1",e),this.selectedTab.tabName="C1"}catch(t){throw console.error("Hlafware POS Error ==> ",t),t}}async handleAppData(){if(this.appData.appData.pos_profile==null)throw frappe.set_route("Form","POS Profile"),new Error("there is no pos profile");if(this.appData.appData.pos_profile.taxes_and_charges!=null&&this.appData.appData.pos_profile.taxes_and_charges!=""&&(this.taxes_and_charges_template=await this.dataHandler.fetchSalesTaxesAndChargesTemplate(this.appData.appData.pos_profile.taxes_and_charges),this.taxes_and_charges=this.taxes_and_charges_template.taxes),this.appData.appData.pos_profile.company!=null&&this.appData.appData.pos_profile.company!=""&&(this.company=await this.dataHandler.fetchCompany(this.appData.appData.pos_profile.company)),this.appData.appData.customers.length>0)this.defaultCustomer=structuredClone(this.appData.appData.customers[0]);else throw frappe.warn("You dont have a customer","please create a customer to continue",()=>{frappe.set_route("Form","Customer")},"Create",!1),new Error("there is no customer");if(this.appData.appData.price_lists.length>0)this.defaultPriceList.name=this.appData.appData.pos_profile.selling_price_list;else throw frappe.warn("You dont have a single price list","please create a priceList to continue",()=>{frappe.set_route("Form","Price List")},"Create",!1),new Error("there is no price list")}prepare_container(){this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/selectorBox.css">'),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/checkInOutCart.css">'),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/itemDetailsCart.css">'),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/paymentMethodCart.css">'),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/customerBox.css">'),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/cartBox.css">'),this.wrapper.append('<link rel="stylesheet" type="text/css" href="/assets/pos_ar/css/historyCarts.css">'),this.wrapper.append('<div id="MainContainer" class="rowBoxReverse"></div>'),this.$components_wrapper=this.wrapper.find("#MainContainer")}prepare_components(){this.set_right_and_left_sections(),this.init_item_selector(),this.init_customer_box(),this.init_selected_item(),this.init_item_details(),this.init_paymentCart(),this.init_historyCart(),this.init_checkInOutCart(),this.init_debtCart(),this.init_settingsCart(),this.init_unsyncedPosCart()}async checkForPOSEntry(){let t=frappe.session.user,e=frappe.defaults.get_default("POS Profile");if(!e){this.wrapper.html("");let i=$(`
			...
			`);return this.wrapper.append(i),!1}let s=await frappe.db.get_doc("POS Profile",e);this.appData.appData.pos_profile=s;try{let i=await frappe.call({method:"pos_ar.pos_ar.doctype.pos_info.pos_info.check_opening_entry",args:{user:t,posProfile:e}});console.log("the result is :::: ",i," user : ",t," posProfile : ",e);let n=i.message;return n.length===0?(this.create_opening_voucher(),!1):(Object.assign(this.POSOpeningEntry,n[0]),this.db.updateCheckInOutSync(this.POSOpeningEntry.period_start_date),!0)}catch(i){return console.error("error occured : ",i),frappe.throw("Error checking for POS Opening Entry."),!1}}create_opening_voucher(){let t=this,e=[5,10,20,50,100,200,500,1e3,2e3],s=frappe.defaults.get_default("company"),i=frappe.defaults.get_default("pos_profile"),n=null,a=()=>{if(!i){frappe.msgprint({title:__("Error"),message:__("Default POS Profile is not set."),indicator:"red"});return}frappe.db.get_doc("POS Profile",i).then(({payments:c})=>{if(console.log("Available payment methods:",c),!c||c.length===0){frappe.msgprint({title:__("No Payment Methods"),message:__("No payment methods found for the default POS Profile."),indicator:"orange"});return}let l=c.find(d=>d.default);l?(n=l.mode_of_payment,console.log("Default payment method:",n)):frappe.msgprint({title:__("Error"),message:__("No default payment method is set."),indicator:"red"})}).catch(c=>{console.error("Error fetching POS Profile:",c),frappe.msgprint({title:__("Error"),message:__("Failed to fetch payment methods. Please try again."),indicator:"red"})})},o=[{fieldname:"denomination",fieldtype:"Data",label:"Denomination (DA)",in_list_view:1,read_only:1},{fieldname:"quantity",fieldtype:"Int",label:"Quantity",in_list_view:1,default:0,onchange:function(){console.log("on change");let c=r.fields_dict.denomination_details.grid.get_data().reduce((l,d)=>l+parseInt(d.denomination)*(d.quantity||0),0);r.fields_dict.total_amount.set_value(c)},onblur:function(){console.log("on blur");let c=r.fields_dict.denomination_details.grid.get_data().reduce((l,d)=>l+parseInt(d.denomination)*(d.quantity||0),0);r.fields_dict.total_amount.set_value(c)}}],r=new frappe.ui.Dialog({title:__("Create POS Opening Entry"),static:!0,fields:[{fieldname:"denomination_details",fieldtype:"Table",label:"Denomination Details",cannot_add_rows:!1,in_place_edit:!0,reqd:1,data:e.map(c=>({denomination:c.toString(),quantity:0})),fields:o},{fieldtype:"Currency",fieldname:"total_amount",label:"Total Amount",read_only:1,default:0}],primary_action:async function(){let c=r.fields_dict.total_amount.get_value();if(!n){frappe.msgprint({title:__("Error"),message:__("No default payment method set. Please configure it in POS Profile."),indicator:"red"});return}if(c<=0)return frappe.show_alert({message:__("Please enter quantities for denominations."),indicator:"red"}),frappe.utils.play_sound("error");let l=[{mode_of_payment:n,opening_amount:c}],d="erpnext.selling.page.point_of_sale.point_of_sale.create_opening_voucher",h=await frappe.call({method:d,args:{pos_profile:i,company:s,balance_details:l},freeze:!0});h.exc||(t.prepare_app_data(h.message),Object.assign(t.POSOpeningEntry,{name:h.message.name,pos_profile:h.message.pos_profile,period_start_date:h.message.period_start_date,company:h.message.company}),t.db.updateCheckInOutSync(t.POSOpeningEntry.period_start_date),t.check_in_out_cart.getAllCheckInOut(),r.hide())},primary_action_label:__("Submit")});r.show(),r.get_field("denomination_details").grid.wrapper.on("keydown","input[data-fieldname='quantity']",c=>{if(console.log("Keydown event outside => ",c.key,"which : ",c.which),c.which===40||c.which===38){console.log("Keydown event:",c.key);let l=r.get_field("denomination_details").grid,d=$(c.target).closest(".grid-row"),h=l.get_row(d.attr("data-idx")-1).doc,p=parseInt(c.target.value)||0;console.log("Saving value:",p,"for denomination:",h.denomination),h.quantity=p,l.refresh_field("quantity");let m=l.get_data().reduce((_,u)=>_+parseInt(u.denomination)*(u.quantity||0),0);r.fields_dict.total_amount.set_value(m)}}),a()}set_right_and_left_sections(){this.$components_wrapper.append('<div id="LeftSection" class="columnBoxReverse"></div>'),this.$components_wrapper.append('<div id="RightSection" class="columnBox"></div>'),this.$rightSection=this.$components_wrapper.find("#RightSection"),this.$leftSection=this.$components_wrapper.find("#LeftSection")}init_customer_box(){this.customer_box=new pos_ar.PointOfSale.pos_customer_box(this.$leftSection,this.appData.appData.customers,this.defaultCustomer,()=>{this.screenManager.navigate("home")},this.onSync.bind(this),this.saveCheckInOut.bind(this),this.onMenuClick.bind(this),()=>{this.screenManager.navigate("debt_cart")},t=>{this.history_cart.print_receipt(t)}),this.screenManager.registerScreen("customer_box",this.customer_box),this.screenManager.customer_box=this.customer_box}init_item_selector(){this.item_selector=new pos_ar.PointOfSale.pos_item_selector(this.$leftSection,this.appData.appData,this.appData.appData.items,this.appData.appData.item_barcodes,this.appData.appData.item_groups,this.appData.appData.item_prices,this.settings_data.settings,this.defaultPriceList,this.auto_select.bind(this),t=>{this.itemClick_selector(t)}),this.screenManager.registerScreen("item_selector",this.item_selector),this.screenManager.item_selector=this.item_selector}init_selected_item(){this.selected_item_cart=new pos_ar.PointOfSale.pos_selected_item_cart(this.$rightSection,this.settings_data,this.selectedItemMaps,this.appData.appData.price_lists,this.appData.appData.customers,this.appData.brands,this.taxes_and_charges,this.invoiceData,this.selectedTab,this.selectedItem,this.selectedField,t=>{this.onSelectedItemClick(t)},t=>{this.item_selector.clearSearchField(),this.screenManager.navigate("home")},(t,e)=>{this.onKeyPressed(t,e)},this.createNewTab.bind(this),()=>{this.savePosInvoice(),this.screenManager.navigate("payment_cart")},this.savePosInvoice.bind(this),this.db),this.screenManager.registerScreen("selected_item_cart",this.selected_item_cart),this.screenManager.selected_item_cart=this.selected_item_cart}init_item_details(){this.item_details=new pos_ar.PointOfSale.pos_item_details(this.$leftSection,this.appData.appData.pos_profile.warehouse,this.appData.appData.price_lists,this.appData.appData.item_prices,this.selectedItem,this.selectedField,(t,e,s)=>{this.onInput(t,e,s)},()=>{this.screenManager.navigate("home")}),this.screenManager.registerScreen("item_details",this.item_details),this.screenManager.item_details=this.item_details}init_paymentCart(){this.payment_cart=new pos_ar.PointOfSale.pos_payment_cart(this.$leftSection,this.selectedItemMaps,this.selectedTab,this.appData.appData,this.appData.appData.pos_profile.payments,this.selectedPaymentMethod,this.invoiceData,()=>{this.screenManager.navigate("home")},this.onCompleteOrder.bind(this),(t,e,s)=>{this.onInput(t,e,s)}),this.screenManager.registerScreen("payment_cart",this.payment_cart),this.screenManager.payment_cart=this.payment_cart}init_historyCart(){this.history_cart=new pos_ar.PointOfSale.pos_history(this.wrapper,this.db,this.appData.appData.pos_profile,this.appData,this.settings_data,this.company,this.taxes_and_charges,this.historyCartClick.bind(this)),this.screenManager.registerScreen("history_cart",this.history_cart),this.screenManager.history_cart=this.history_cart}init_checkInOutCart(){this.check_in_out_cart=new pos_ar.PointOfSale.pos_check_in_out(this.wrapper,this.db),this.screenManager.registerScreen("check_in_out_cart",this.check_in_out_cart),this.screenManager.check_in_out_cart=this.check_in_out_cart}init_debtCart(){this.debt_cart=new pos_ar.PointOfSale.pos_debt_cart(this.wrapper,this.appData,this.POSOpeningEntry,()=>{this.check_in_out_cart.getAllCheckInOut()}),this.screenManager.registerScreen("debt_cart",this.debt_cart),this.screenManager.debt_cart=this.debt_cart}init_settingsCart(){this.settings_cart=new pos_ar.PointOfSale.pos_settings(this.wrapper,this.settings_data,this.appData.appData.pos_profile,this.onSettingsChange.bind(this)),this.screenManager.registerScreen("settings_cart",this.settings_cart),this.screenManager.settings_cart=this.settings_cart}init_unsyncedPosCart(){this.unsynced_pos_cart=new pos_ar.PointOfSale.pos_unsynced_cart(this.wrapper,this.appData,this.db,t=>{let e=this.selected_item_cart.createTabForEditPOS();this.selectedItemMaps.set(`C${e}`,t),this.selectedTab.tabName=`C${e}`,this.screenManager.navigate("home")},()=>{this.unsyncedPos>=1?(this.customer_box.setSynced(),this.unsyncedPos=0):(this.unsyncedPos-=1,this.customer_box.setNotSynced(this.unsyncedPos))}),this.screenManager.registerScreen("unsynced_pos_cart",this.unsynced_pos_cart),this.screenManager.unsynced_pos_cart=this.unsynced_pos_cart}itemClick_selector(t,e){this.syncInput=!1;let s=structuredClone(t);s.discount_amount=0,s.discount_percentage=0,Object.assign(this.selectedItem,s),this.addItemToPosInvoice(s),this.selected_item_cart.calculateNetTotal(),this.selected_item_cart.calculateVAT(),this.selected_item_cart.calculateQnatity(),this.selected_item_cart.calculateGrandTotal(),this.selected_item_cart.refreshSelectedItem(),e&&this.item_selector.refreshItemSelector(),this.selectedField.field_name="quantity",this.selected_item_cart.makeSelectedButtonHighlighted(),this.selected_item_cart.scrollToBottom(),this.savePosInvoice()}onSelectedItemClick(t){this.syncInput=!1;let e=structuredClone(t);Object.assign(this.selectedItem,e),this.selectedField.field_name="quantity",this.selected_item_cart.makeSelectedButtonHighlighted(),console.log("selected item :: ",this.selectedItem),this.screenManager.navigate("item_details"),this.item_details.refreshDate(t)}saveCheckInOut(t){t.is_sync=0,this.appData.saveCheckInOut(t,e=>{this.check_in_out_cart.getAllCheckInOut()},e=>{console.log("err to save checkInOut : ",e)})}onSettingsChange(t){this.item_selector.refreshItemSelector(),this.selected_item_cart.resetItemRateBaseOnPriceList(),this.selected_item_cart.refreshSelectedItem()}savePosInvoice(t){this.selectedItemMaps.get(this.selectedTab.tabName).synced=!1,this.appData.savePosInvoice(this.selectedItemMaps.get(this.selectedTab.tabName))}saveThatPosInvoice(t){t.synced=!1,this.appData.savePosInvoice(t)}auto_select(t){this.itemClick_selector(t),this.item_selector.refresh()}onMenuClick(t){t=="recent_pos"?this.screenManager.navigate("history_cart"):t=="close_pos"?this.onClosePOS():t=="unsenced_invoices"?this.screenManager.navigate("unsynced_pos_cart"):t=="settings"?this.screenManager.navigate("settings_cart"):t=="checkInOut"&&this.screenManager.navigate("check_in_out_cart")}getDefaultPaymentMethod(){let t=null;return this.appData.appData.pos_profile.payments.forEach(e=>{e.default&&(t=e)}),t}getPaymentMethods(){let t=[];return this.appData.appData.pos_profile.payments.forEach(e=>{t.push({mode_of_payment:e.mode_of_payment,default:e.default,amount:0})}),t}restorePosInvoices(t){t.forEach(e=>{let s=this.selected_item_cart.createTabForEditPOS(),i=frappe.model.get_new_doc("POS Invoice");i.customer=e.customer,i.pos_profile=this.appData.appData.pos_profile.name,i.items=e.items,i.taxes_and_charges=e.taxes_and_charges,i.additional_discount_percentage=e.additional_discount_percentage,i.paid_amount=e.paid_amount,i.base_paid_amount=e.base_paid_amount,i.creation_time=e.creation_time,i.payments=e.payments,i.is_pos=1,i.update_stock=1,i.docstatus=0,i.synced=!1,i.status="Draft",i.priceList=e.priceList,i.opened=1;let n=new Date,[a,o,r]=n.toISOString().split("T")[0].split("-"),c=n.getHours(),l=n.getMinutes(),d=n.getMilliseconds();i.refNum=this.appData.appData.pos_profile.name+"-"+a+"-"+o+"-"+r+"-"+c+l+d,i.custom_cach_name=i.refNum,this.selectedItemMaps.set(`C${s}`,i),this.selectedTab.tabName=`C${s}`,this.saveThatPosInvoice(i)}),this.screenManager.navigate("home")}createNewTab(t){this.item_selector.clearSearchField();let e=frappe.model.get_new_doc("POS Invoice");e.customer=this.defaultCustomer.name,e.pos_profile=this.appData.appData.pos_profile.name,e.items=[],e.taxes_and_charges=this.appData.appData.pos_profile.taxes_and_charges,e.additional_discount_percentage=0,e.paid_amount=0,e.base_paid_amount=0,e.creation_time=frappe.datetime.now_datetime(),e.payments=this.getPaymentMethods(),e.is_pos=1,e.update_stock=1,e.docstatus=0,e.synced=!1,e.status="Draft",e.priceList=this.defaultPriceList.name,e.opened=1;let s=new Date,[i,n,a]=s.toISOString().split("T")[0].split("-"),o=s.getHours(),r=s.getMinutes(),c=s.getMilliseconds();e.refNum=this.appData.appData.pos_profile.name+"-"+i+"-"+n+"-"+a+"-"+o+r+c,e.custom_cach_name=e.refNum,this.selectedItemMaps.set(`C${t}`,e),this.selectedTab.tabName=`C${t}`,this.screenManager.navigate("home")}historyCartClick(t,e){if(t=="edit"){let s=this.selected_item_cart.createTabForEditPOS();this.selectedItemMaps.set(`C${s}`,e),this.selectedTab.tabName=`C${s}`,this.screenManager.navigate("home")}else if(t=="duplicate"){this.item_selector.clearSearchField();let s=this.selected_item_cart.createNewTab();this.selectedItemMaps.get(this.selectedTab.tabName).items=e.items,console.log("message :: ",e),console.log("see here ",this.selectedItemMaps.get(this.selectedTab.tabName),"selected tab ",this.selectedTab.tabName),this.screenManager.navigate("home")}else if(t=="return"){let s=this.selected_item_cart.createTabForEditPOS(),i=this.makePosInvoiceReturn(e);this.selectedItemMaps.set(`C${s}`,i),this.selectedTab.tabName=`C${s}`,this.screenManager.navigate("home")}}onInput(t,e,s){if(t=="focus"||t=="blur"){t=="focus"&&Object.assign(this.selectedField,{field_name:e}),t=="blur"&&Object.assign(this.selectedField,{field_name:null}),this.item_details.makeSelectedFieldHighlighted(),this.selected_item_cart.makeSelectedButtonHighlighted();return}if(e=="quantity")this.selectedItem.qty=parseFloat(s),this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty),this.selected_item_cart.refreshSelectedItem();else if(e=="rate"){this.selectedItem.rate=parseFloat(s);let i=this.selectedItem.rate,n=this.selectedItem.discount_percentage,a=i*(n/100);this.selectedItem.discount_percentage=n,this.selectedItem.discount_amount=a,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate),this.selected_item_cart.refreshSelectedItem(),this.item_details.refreshDate(this.selectedItem)}else if(e=="discount_percentage"){let n=this.selectedItem.rate*(parseFloat(s)/100);this.selectedItem.discount_percentage=parseFloat(s),this.selectedItem.discount_amount=n,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemDiscountPercentage(this.selectedItem.name,this.selectedItem.discount_percentage),this.selected_item_cart.refreshSelectedItem(),this.item_details.refreshDate(this.selectedItem)}else if(e=="discount_amount"){let i=this.selectedItem.rate,n=(parseFloat(s)*100/i).toFixed(2),a=parseFloat(s);n>100&&(n=100),parseFloat(s)>i&&(a=i),this.selectedItem.discount_percentage=parseFloat(n),this.selectedItem.discount_amount=a,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemDiscountPercentage(this.selectedItem.name,this.selectedItem.discount_percentage),this.selected_item_cart.refreshSelectedItem(),this.item_details.refreshDate(this.selectedItem)}this.savePosInvoice()}onKeyPressed(t,e){var s,i;if(t=="quantity")this.selectedField.field_name="quantity",this.selected_item_cart.makeSelectedButtonHighlighted();else if(t=="rate")this.selectedField.field_name="rate",this.selected_item_cart.makeSelectedButtonHighlighted();else if(t=="minus"){this.syncInput=!0;let n=parseFloat(this.selectedItem.qty)*-1;this.selectedItem.qty=parseFloat(n),this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty)}else if(t=="print"){this.history_cart.print_receipt(structuredClone(this.selectedItemMaps.get(this.selectedTab.tabName)));return}else if(t=="remove")this.syncInput=!1,this.deleteItemFromPOsInvoice(this.selectedItem.name);else if(t=="delete"){if(!this.settings_data.settings.showItemDetails){if(this.selectedField.field_name=="quantity"){let o=`${parseFloat(this.selectedItem.qty)}`.slice(0,-1);this.selectedItem.qty=parseFloat(o)||0,this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty)}else if(this.selectedField.field_name=="rate"){let o=`${parseFloat(this.selectedItem.rate)}`.slice(0,-1);this.selectedItem.rate=parseFloat(o)||0;let r=this.selectedItem.rate,c=this.selectedItem.discount_percentage,l=r*(c/100);this.selectedItem.discount_percentage=c,this.selectedItem.discount_amount=l,this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate)}else if(this.selectedField.field_name=="discount_percentage"){let a=this.selectedItem.rate,r=`${(s=this.selectedItem.discount_percentage)!=null?s:0}`+e,c=parseFloat(r);c>100&&(c=100);let l=a*(c/100),d=a-l;this.selectedItem.discount_percentage=c,this.selectedItem.discount_amount=l,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate)}}let n=parseFloat(this.item_details.deleteCharacter());if(this.selectedField.field_name=="quantity")this.selectedItem.qty=n,this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty);else if(this.selectedField.field_name=="rate"){this.selectedItem.rate=n;let a=this.selectedItem.rate,o=this.selectedItem.discount_percentage,r=a*(o/100);this.selectedItem.discount_amount=r,this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate)}else if(this.selectedField.field_name=="discount_percentage"){let o=this.selectedItem.rate*(n/100);this.selectedItem.discount_percentage=n,this.selectedItem.discount_amount=o,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate)}else this.selectedField.field_name=="cash"&&this.payment_cart.deleteKeyPress()}else if(t=="addToField"){if(this.selectedField.field_name=="cash")this.payment_cart.handleInput(e);else if(this.selectedField.field_name=="quantity"){let n=0;this.syncInput?n=parseFloat(this.selectedItem.qty):(n=0,this.syncInput=!0);let a=`${n}`+e;this.selectedItem.qty=parseFloat(a),this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty)}else if(this.selectedField.field_name=="rate"){let n=0;this.syncInput?n=parseFloat(this.selectedItem.rate):(n=0,this.syncInput=!0);let a=`${n}`+e;this.selectedItem.rate=parseFloat(a);let o=this.selectedItem.rate,r=this.selectedItem.discount_percentage,c=o*(r/100);this.selectedItem.discount_percentage=r,this.selectedItem.discount_amount=c,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate)}else if(this.selectedField.field_name=="discount_percentage"){let n=this.selectedItem.rate,o=`${(i=this.selectedItem.discount_percentage)!=null?i:0}`+e,r=parseFloat(o);r>100&&(r=100);let c=n*(r/100),l=n-c;this.selectedItem.discount_percentage=r,this.selectedItem.discount_amount=c,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate)}}this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.selected_item_cart.refreshSelectedItem(),this.item_details.refreshDate(this.selectedItem),this.savePosInvoice()}onCompleteOrder(){if(this.payment_cart.show_waiting(),this.savePosInvoice(),this.defaultCustomer.name==""){frappe.warn("Customer didnt selected!","you have to select a customer",()=>{},"Done",!1);return}let t=[],e=1;if(this.selectedItemMaps.get(this.selectedTab.tabName).items.forEach(a=>{let o={item_name:a.item_name,item_code:a.item_code,rate:a.rate,qty:a.qty,description:a.description,image:a.image,use_serial_batch_fields:1,cost_center:this.appData.appData.pos_profile.cost_center,discount_percentage:a.discount_percentage,discount_amount:a.discount_amount,warehouse:this.appData.appData.pos_profile.warehouse,income_account:this.appData.appData.pos_profile.income_account};t.push(o),a.qty>0&&(e=0)}),this.selectedItemMaps.get(this.selectedTab.tabName).is_return=e,this.selectedItemMaps.get(this.selectedTab.tabName).items=t,t.length==0)return;let s=0;this.selectedItemMaps.get(this.selectedTab.tabName).items.forEach(a=>{s+=a.rate*a.qty}),this.calculatePaidAmount(this.selectedItemMaps.get(this.selectedTab.tabName))>s?(this.selectedItemMaps.get(this.selectedTab.tabName).paid_amount=s,this.selectedItemMaps.get(this.selectedTab.tabName).base_paid_amount=s,this.selectedItemMaps.get(this.selectedTab.tabName).outstanding_amount=0,this.selectedItemMaps.get(this.selectedTab.tabName).total_customer_payment=this.calculatePaidAmount(this.selectedItemMaps.get(this.selectedTab.tabName))):(this.selectedItemMaps.get(this.selectedTab.tabName).paid_amount=this.calculatePaidAmount(this.selectedItemMaps.get(this.selectedTab.tabName)),this.selectedItemMaps.get(this.selectedTab.tabName).base_paid_amount=this.calculatePaidAmount(this.selectedItemMaps.get(this.selectedTab.tabName)),this.selectedItemMaps.get(this.selectedTab.tabName).outstanding_amount=this.invoiceData.grandTotal-this.calculatePaidAmount(this.selectedItemMaps.get(this.selectedTab.tabName)),this.selectedItemMaps.get(this.selectedTab.tabName).total_customer_payment=this.calculatePaidAmount(this.selectedItemMaps.get(this.selectedTab.tabName))),this.selectedItemMaps.get(this.selectedTab.tabName).docstatus=1;let i=this.checkIfPaid(this.selectedItemMaps.get(this.selectedTab.tabName));this.selectedItemMaps.get(this.selectedTab.tabName).status=i;let n=structuredClone(this.selectedItemMaps.get(this.selectedTab.tabName));n.custom_is_shared=this.settings_data.settings.sendInvoiceToOtherPos?1:0,i=="Unpaid"?(n.synced=!0,frappe.db.insert(n).then(a=>{this.payment_cart.hide_waiting(),n.opened=0,n.real_name=a.name,this.history_cart.print_receipt(n),this.appData.updatePosInvoice(n),this.selectedItemMaps.delete(this.selectedTab.tabName),Array.from(this.selectedItemMaps.keys()).length>0?this.selected_item_cart.createNewTab():this.selected_item_cart.createNewTab(),this.screenManager.navigate("home")}).catch(a=>{this.payment_cart.hide_waiting(),console.log("cant push pos invoice : ",a)})):(this.payment_cart.hide_waiting(),this.history_cart.print_receipt(n),this.selectedItemMaps.delete(this.selectedTab.tabName),Array.from(this.selectedItemMaps.keys()).length>0?this.selected_item_cart.createNewTab():this.selected_item_cart.createNewTab(),this.screenManager.navigate("home"),n.synced=!1,n.opened=0,this.appData.updatePosInvoice(n),this.unsyncedPos+=1,this.customer_box.setNotSynced(this.unsyncedPos))}onSync(){if(this.isSyncing){console.warn("Sync already in progress.");return}if(this.isSyncing=!0,this.POSOpeningEntry.name==""){this.checkForPOSEntry(),this.isSyncing=!1;return}if(this.unsyncedPos<=0){frappe.msgprint({title:__("Sync Complete"),indicator:"green",message:__("All data is already synchronized.")}),this.isSyncing=!1;return}let t=0,e=0;this.appData.getNotSyncedPos(s=>{frappe.show_progress("Syncing Invoices...",0,s.length,"syncing"),s.forEach(i=>{frappe.db.insert(i).then(n=>{let a=structuredClone(i);a.synced=!0,a.real_name=n.name,this.appData.updatePosInvoice(a),t+=1,frappe.show_progress("Syncing Invoices...",t,s.length,"syncing"),t==s.length&&(frappe.hide_progress(),this.customer_box.setSynced(),this.isSyncing=!1,this.unsyncedPos=0)}).catch(n=>{t+=1,e+=1,t==s.length&&(frappe.hide_progress(),this.customer_box.setSynced(),this.isSyncing=!1,this.unsyncedPos=0)})})},s=>{console.log("cant get the unseced POS from local"),this.isSyncing=!1})}getCustomerDefaultPriceList(t){let e="";return this.appData.appData.customers.forEach(s=>{s.name==t&&(e=s.default_price_list)}),e}checkIfPaid(t){let e=0,s=0,i=0,n=0;return t.items.forEach(a=>{e+=a.qty*a.rate}),this.taxes_and_charges.forEach(a=>{i+=a.rate/100*e}),n=t.additional_discount_percentage/100*e,s=e+i-n,t.paid_amount==0||t.paid_amount<s?"Unpaid":"Paid"}checkIfRateZero(t){return t.items.some(e=>e.rate==0)}onClosePOS(){this.unsyncedPos>0&&frappe.throw(__("you have  some invoices to sync first."));let t=frappe.model.get_new_doc("POS Closing Entry");t.pos_opening_entry=this.POSOpeningEntry.name,t.pos_profile=this.POSOpeningEntry.pos_profile,t.company=this.POSOpeningEntry.company,t.user=frappe.session.user,this.check_in_out_cart.checkList.filter(i=>i.is_sync===0).forEach(i=>{let n=frappe.model.add_child(t,"check_in_out","custom_check_in_out");n.check_type=i.check_type,n.creation_time=i.creation_time,n.amount=i.amount,n.reason_note=i.reason_note,n.user=i.owner}),this.POSOpeningEntry.name="",this.wrapper.html("");let s=$(`
			<div class="pos-profile-dialog d-flex flex-column align-items-center justify-content-center" style="height: 100vh;">
				<div class="alert alert-warning text-center">
		...
		`);this.wrapper.append(s),frappe.set_route("Form","POS Closing Entry",t.name).then(()=>{window.addEventListener("popstate",i=>{frappe.set_route("Form","POS Closing Entry",t.name)})})}setListeners(){window.addEventListener("offline",function(){frappe.msgprint("you lose the connection (offline mode)")}),window.addEventListener("online",function(){frappe.msgprint("the connection is back (online mode)")})}makePosInvoiceReturn(t){let e=structuredClone(t),s=frappe.model.get_new_doc("POS Invoice").name;e.name=s,e.is_return=1,e.return_against=t.real_name,e.real_name="",e.consolidated_invoice=null,e.outstanding_amount=0;let i=[];e.items.forEach(h=>{if(h.qty>0){let p=structuredClone(h);p.qty=p.qty*-1,p.description="Returned item",i.push(p)}}),e.items=i,e.pos_profile=this.appData.appData.pos_profile.name,e.taxes_and_charges=this.appData.appData.pos_profile.taxes_and_charges,e.creation_time=frappe.datetime.now_datetime(),e.payments=this.getPaymentMethods(),e.priceList=this.defaultPriceList.name;let n=new Date,[a,o,r]=n.toISOString().split("T")[0].split("-"),c=n.getHours(),l=n.getMinutes(),d=n.getMilliseconds();return e.refNum=this.appData.appData.pos_profile.name+"-"+a+"-"+o+"-"+r+"-"+c+l+d,e.custom_cach_name=e.refNum,e}toggleKeyboardMode(t){t?document.addEventListener("keydown",e=>{let s=document.activeElement;if(!(s.tagName==="INPUT"||s.tagName==="TEXTAREA"||s.isContentEditable)){if(e.key=="q"||e.key=="Q")this.selectedField.field_name="quantity",this.selected_item_cart.makeSelectedButtonHighlighted();else if(e.key=="p"||e.key=="P")this.selectedField.field_name="rate",this.selected_item_cart.makeSelectedButtonHighlighted();else if(e.key=="Delete")this.deleteItemFromPOsInvoice(this.selectedItem.name),this.selected_item_cart.refreshSelectedItem(),this.screenManager.navigate("home");else if(e.key=="Backspace"){if(this.selectedField.field_name=="quantity"){let n=parseFloat(this.selectedItem.qty),a=0;a=parseFloat(`${n}`.slice(0,-1))||0,this.selectedItem.qty=parseFloat(a),this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty),this.selected_item_cart.refreshSelectedItem()}else if(this.selectedField.field_name=="rate"){let n=parseFloat(this.selectedItem.rate),a=parseFloat(`${n}`.slice(0,-1))||0;this.selectedItem.rate=parseFloat(a);let o=this.selectedItem.rate,r=this.selectedItem.discount_percentage,c=o*(r/100);this.selectedItem.discount_percentage=r,this.selectedItem.discount_amount=c,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate),this.selected_item_cart.refreshSelectedItem(),this.item_details.refreshDate(this.selectedItem)}}else if(this.selectedField.field_name=="quantity"){if(parseFloat(e.key)||e.key=="0"){let n=0;this.syncInput?n=parseFloat(this.selectedItem.qty):(n=0,this.syncInput=!0);let a=`${n}`+e.key;this.selectedItem.qty=parseFloat(a),this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty),this.selected_item_cart.refreshSelectedItem()}else if(e.key=="-"){this.syncInput=!0;let n=parseFloat(this.selectedItem.qty)*-1;this.selectedItem.qty=parseFloat(n),this.editPosItemQty(this.selectedItem.name,this.selectedItem.qty),this.selected_item_cart.refreshSelectedItem()}}else if(this.selectedField.field_name=="rate"&&(parseFloat(e.key)||e.key=="0")){let n=0;this.syncInput?n=parseFloat(this.selectedItem.rate):(n=0,this.syncInput=!0);let a=`${n}`+e.key;this.selectedItem.rate=parseFloat(a);let o=this.selectedItem.rate,r=this.selectedItem.discount_percentage,c=o*(r/100);this.selectedItem.discount_percentage=r,this.selectedItem.discount_amount=c,this.editPosItemDiscountAmount(this.selectedItem.name,this.selectedItem.discount_amount),this.editPosItemRate(this.selectedItem.name,this.selectedItem.rate),this.selected_item_cart.refreshSelectedItem(),this.item_details.refreshDate(this.selectedItem)}}}):document.removeEventListener("keydown",e=>{})}checkServiceWorker(){"serviceWorker"in navigator&&(window.addEventListener("DOMContentLoaded",()=>{navigator.serviceWorker.register("./sw.js").then(t=>console.log("Service Worker registered successfully.")).catch(t=>console.log(`Service Worker registration failed: ${t}`))}),this.sw=new pos_ar.PointOfSale.Sw,document.readyState==="complete"&&navigator.serviceWorker.register("../assets/pos_ar/public/js/sw.js").then(t=>console.log("Service Worker registered successfully.")).catch(t=>console.log(`Service Worker registration failed: ${t}`)))}checkUnSyncedPos(){this.appData.getNotSyncedPosNumber(t=>{this.unsyncedPos=t,this.unsyncedPos==0?this.customer_box.setSynced(t):this.customer_box.setNotSynced(t)},t=>{console.log(`error occured when check unSynced POS : ${t} `)})}addItemToPosInvoice(t){var a;let e=structuredClone(t),i=this.selectedItemMaps.get(this.selectedTab.tabName).items,n=!1;if(i.forEach(o=>{if(o.name==t.name){n=!0,o.qty+=1;let r=structuredClone(o);Object.assign(this.selectedItem,r)}}),!n){e.item_code=e.name,e.discount_amount=0,e.discount_percentage=0,e.qty=1,e.rate=((a=e.prices.find(r=>r.price_list==this.selectedItemMaps.get(this.selectedTab.tabName).priceList))==null?void 0:a.price_list_rate)||0,i.push(e);let o=structuredClone(e);Object.assign(this.selectedItem,o)}}deleteItemFromPOsInvoice(t){let e=this.selectedItemMaps.get(this.selectedTab.tabName),s=e.items;e.items=s.filter(i=>i.name!=t),this.selectedItem=structuredClone({name:""})}editPosItemQty(t,e){this.selectedItemMaps.get(this.selectedTab.tabName).items.forEach(i=>{i.name==t&&(i.qty=e)})}editPosItemRate(t,e){this.selectedItemMaps.get(this.selectedTab.tabName).items.forEach(i=>{i.name==t&&(i.rate=e,i.manually_edited=!0)})}editPosItemDiscountPercentage(t,e){this.selectedItemMaps.get(this.selectedTab.tabName).items.forEach(i=>{i.name==t&&(i.discount_percentage=e)})}editPosItemDiscountAmount(t,e){this.selectedItemMaps.get(this.selectedTab.tabName).items.forEach(i=>{i.name==t&&(i.discount_amount=e)})}calculatePaidAmount(t){let e=0;return t.payments.forEach(s=>{e+=s.amount}),e}};pos_ar.PointOfSale.pos_item_selector=class{constructor(t,e,s,i,n,a,o,r,c,l){this.wrapper=t,this.app_data=e,this.item_list=s,this.item_barcodes=i,this.item_group_list=n,this.item_prices=a,this.settings_data=o,this.selected_price_list=r,this.auto_select=c,this.on_item_click=l,this.barcode_map={},this.item_barcodes.forEach(d=>{this.barcode_map[d.barcode]||(this.barcode_map[d.barcode]=[]),this.barcode_map[d.barcode].push(d.parent)}),this.start_item_selector()}start_item_selector(){this.prepare_select_box(),this.setItemGroupsInList(),this.setItemInFlow(this.getItemByItemGroup("")),this.setListeners()}refresh(){this.setItemInFlow(this.getItemByItemGroup(""))}prepare_select_box(){this.wrapper.append('<div id="SelectorBox" class="columnBox" ></div>'),this.selectorBox=this.wrapper.find("#SelectorBox"),this.selectorBox.append('<div id="selectorBoxHeader" class="rowBox header"></div>'),this.header=this.selectorBox.find("#selectorBoxHeader"),this.header.append('<h4 class="CartTitle">Items</h4>'),this.header.append('<div id="inputsBox" class="rowBox align_center"  style="flex-grow: 1; "></div>'),this.inputBox=this.header.find("#inputsBox"),this.inputBox.append(`
		  ...
		  `),this.inputBox.append('<input list="ItemGroupList"  id="ItemGroupInput" name="ItemGroup" placeHolder="Item Group">'),this.inputBox.append('<datalist id="ItemGroupList"></datalist>'),this.itemGroupList=this.inputBox.find("#ItemGroupList"),this.itemGroupList.append("<option>fetching Item Groups ...</option>"),this.settings_data.search_by_group||this.inputBox.find("#ItemGroupInput").hide(),this.selectorBox.append('<div id="itemsContainer" class="rowBox row_wrap"></div>'),this.itemsContainer=this.selectorBox.find("#itemsContainer")}setItemGroupsInList(){let t=document.getElementById("ItemGroupList");t.innerHTML="",this.item_group_list.forEach(e=>{let s=document.createElement("option");s.value=e.name,s.textContent=e.customer_name,t.appendChild(s)})}refreshItemSelector(){let t=document.getElementById("ItemInput");t.value="";let e=document.getElementById("ItemGroupInput");this.setItemInFlow(this.getItemByItemGroup(e.value))}setItemInFlow(t){var i;let e=document.getElementById("itemsContainer");e.innerHTML="";let s=document.getElementById("ItemInput");if(t.length===0){e.innerHTML=`
			...
			`;return}for(let n=0;n<t.length&&n<50;n++){let a=t[n],o=document.createElement("div");o.classList.add("itemBox"),o.classList.add("columnBox"),o.classList.add("C_A_Center"),o.addEventListener("click",l=>{let d=s.value.length>0;this.on_item_click(a,d)});let r=a.image||"/assets/pos_ar/images/no_image.png",c=((i=a.prices.find(l=>l.price_list==this.selected_price_list.name))==null?void 0:i.price_list_rate)||0;o.innerHTML=`
				<img class="itemImage" src="${r}" alt="${a.item_name}" onerror="this.src='/assets/pos_ar/images/no_image.png'">
				<div class="itemTitle">${a.item_name}</div>
				<div class="itemPrice">${c} DA</div>
				<div class="itemQty">${a.stock_qty}</div>
			`,e.appendChild(o)}t.length>=700&&e.insertAdjacentHTML("beforeend",`
				<div class="more-items-notice">
					<p>Showing first 700 items. Please refine your search to see more specific results.</p>
				</div>
			`)}clearSearchField(){let t=document.getElementById("ItemInput");t.value="",this.setItemInFlow(this.item_list)}showCart(){this.selectorBox.css("display","flex")}hideCart(){this.selectorBox.css("display","none")}setListeners(){document.getElementById("ItemGroupInput").addEventListener("input",i=>{this.setItemInFlow(this.getItemByItemGroup(event.target.value))});let e=document.getElementById("ItemInput");e.addEventListener("input",i=>{this.setItemInFlow(this.filterListByItemData(i.target.value))}),document.getElementById("ClearItemInput").addEventListener("click",()=>{e.value="",this.setItemInFlow(this.item_list),e.focus()})}filterListByItemData(t){let e=this.barcode_map[t];if(e&&e.length>0){let s=this.item_list.filter(i=>e.includes(i.name));if(s.length===1){this.auto_select(s[0]);let i=document.getElementById("ItemInput");return i.value="",this.item_list}else if(s.length>1)return s}return this.item_list.filter(s=>s.item_name.toLowerCase().includes(t.toLowerCase()))}getItemByItemGroup(t){let e=[],s=a=>{e.push(a),this.item_group_list.forEach(o=>{o.parent_item_group==a&&(e.push(o.name),o.is_group&&s(o.name))})};s(t);let i=[],n=a=>{this.item_list.forEach(o=>{o.item_group==a&&i.push(o)})};return e.forEach(a=>{n(a)}),i}};pos_ar.PointOfSale.pos_customer_box=class{constructor(t,e,s,i,n,a,o,r,c){this.wrapper=t,this.customers_list=e,this.selected_customer=s,this.back_home=i,this.on_sync=n,this.on_menu_click=o,this.save_check_in_out=a,this.on_debt_click=r,this.on_print_pos=c,this.online=!0,this.show_menu=!1,this.start_work()}start_work(){this.prepare_customer_box(),this.checkForSync(),this.setListeners()}prepare_customer_box(){this.wrapper.append('<div id="ActionsContainerBox">'),this.actionContainer=this.wrapper.find("#ActionsContainerBox"),this.actionContainer.append(`
		...
		`),frappe.db.get_value("Company",frappe.defaults.get_default("company"),"name").then(e=>{(e.message.name==="OPTILENS TIZIOUZOU"||e.message.name==="Tizi")&&(document.getElementById("popupBtn").style.display="flex")}),this.wrapper.append(`
		...
		`,document.head.appendChild(t),this.actionContainer.append(`
				...
				<div class="receipt-container">
					...
					<div class="receipt-header">
						<div class="customer-info">
							<div class="bold">Client: ${t.customer}</div>
							<div style="font-size:10px;">Commande: ${t.refNum}</div>
							<div style="font-size:10px;">Date: ${o}</div>
							<div style="font-size:10px;">Heure: ${r}</div>
						</div>
					</div>
					...
					<table class="receipt-table">
						...
						<tbody>
			`;t.items.forEach(p=>{let m=p.rate*p.qty;e.netTotal+=m,e.totalQty+=p.qty,e.totalItems+=1,l+=`
					<tr>
						<td>${p.item_name}</td>
						<td class="text-right">${p.qty}</td>
						<td class="text-right">${s(p.rate)}</td>
						<td class="text-right">${s(m)}</td>
					</tr>
				`});let d=t.additional_discount_percentage?e.netTotal*t.additional_discount_percentage/100:0;e.grandTotal=e.netTotal-d,l+=`
						</tbody>
					</table>
	
					<div class="totals">
						<div>Quantit\xE9 Totale: ${e.totalQty}</div>
						<div>Remise: ${s(d)} DA</div>
						<div style="margin-top:15px;">
							<div class="bold" style="display:flex; align-items:center; font-size:18px;">
								<div style="width:120px;">Total:</div>
								<div style="flex-grow:1; text-align:center;">${s(e.grandTotal)} DA</div>
							</div>
							<div style="display:flex; align-items:center; font-size:14px;">
								<div style="width:120px;">Total Solde:</div>
								<div style="flex-grow:1; text-align:center;">${s(n)} DA</div>
							</div>
						</div>
					</div>
		...
		`,e.addEventListener("click",()=>{this.customerList.find(".customer-item").removeClass("selected"),e.classList.add("selected"),this.selected_client=structuredClone(t),this.refreshClientDebtPart(t)}),this.customerList.append(e)}showCart(){this.leftContainer.css("display","flex"),this.rightContainer.css("display","flex")}hideCart(){this.leftContainer.css("display","none"),this.rightContainer.css("display","none")}show_waiting(){this.waiting_cart.css("display","flex")}hide_waiting(){this.waiting_cart.css("display","none")}setListener(){this.rightContainer.find("#debt_filterClientList").on("input",t=>{let e=this.rightContainer.find("#debt_filterClientList").val().trim().toLowerCase();this._filtredClientList=this.app_data.appData.customers.filter(s=>s.customer_name.toLowerCase().includes(e)),this.refreshClientPart()}),this.leftContainer.find("#debt_paymentAmount").on("input",t=>{this.payment_amount=parseFloat(this.leftContainer.find("#debt_paymentAmount").val())}),this.pay_selected_invoices_btn.on("click",async()=>{console.log("check the list ==> ",this._selected_invoice),this.paySelectedInvoice()}),this.debtList.on("click",'.invoiceBox input[type="checkbox"]',t=>{let e=$(t.target),s=e.data("invoice-name"),i=e.data("outstanding-amount"),n=e.data("invoice-type"),a={name:s,outstanding_amount:i,type:n};if(e.is(":checked"))this._selected_invoice.some(o=>o.name==s)||this._selected_invoice.push(a);else{let o=this._selected_invoice.findIndex(r=>r.name==s);o>-1&&this._selected_invoice.splice(o,1)}this.refresh_partially_paid()})}refreshClientPart(){this.customerList.html(""),this._filtredClientList.forEach(t=>{this.add_customer_to_list(t)})}refreshTotal(t){this.total_client_debt.text(`Total Debt : ${t} DA`)}refresh_partially_paid(){let t=0;this._selected_invoice.forEach(e=>{t+=e.outstanding_amount}),this.partially_client_debt.text(`Selected Debt : ${t} DA`)}async refreshClientDebtPart(t){this.refreshTotal("Loading ..."),this._selected_invoice=[],this.refresh_partially_paid();let e="width:calc(100% - 40px);height:60px;min-height:60px;border-bottom:2px solid #505050;",s="width:80px;height:35px;color:white;background:green;border-radius:12px;margin:0px 20px;cursor:pointer;",i=0;this.debtList.html(""),console.log("the company : ",this.app_data.appData.pos_profile.company),console.log("the profile : ",this.app_data.appData.pos_profile);let n=await this.app_data.fetchDebts(t.name,this.app_data.appData.pos_profile.company),a=await this.app_data.fetchDebtsSalesInvoices(t.name,this.app_data.appData.pos_profile.company);n.forEach(r=>{i+=r.outstanding_amount;let c=$(`<div  style="${e}" class="rowBox C_A_Center invoiceBox" data-invoice-name="${r.name}"></div>`),l=$(`<input type="checkbox" class="select_checkbox" style="margin:0px 16px;" data-invoice-type="POS Invoice" data-invoice-name="${r.name}" data-outstanding-amount="${r.outstanding_amount}" ></input>`);c.append(`<div style="flex-grow:1;">${r.name}</div>`),c.append(`<div style="flex-grow:1;">${r.outstanding_amount} DA</div>`),c.append(`<div style="flex-grow:1;">${r.posting_date}</div>`),c.append('<div style="flex-grow:1;">POS Invoice</div>'),c.append(`<div class="rowBox centerItem payBtn" style="${s}">Pay</div>`),c.find(".payBtn").on("click",async()=>{await this.payPosInvoice(r)}),this.debtList.append(c)});let o=this;a.forEach(r=>{i+=r.outstanding_amount;let c=$(`
				<div style="${e}; overflow:hidden; flex-direction: column; transition:0.3s;" 
					 class="rowBox C_A_Center invoiceBox" data-invoice-name="${r.name}">
				</div>
			`),l=$(`
				<div class="rowBox C_A_Center" style="width: 100%;">
				</div>
			`);l.append(`<input type="checkbox" style="margin:0px 16px;" data-invoice-type="Sales Invoice" data-invoice-name="${r.name}" data-outstanding-amount="${r.outstanding_amount}">`),l.append(`<div style="flex-grow:1;">${r.name}</div>`),l.append(`<div style="flex-grow:1;">${r.outstanding_amount} DA</div>`),l.append(`<div style="flex-grow:1;">${r.posting_date}</div>`),l.append('<div style="flex-grow:1;">Sales Invoice</div>'),l.append(`
