...
@frappe.whitelist()
def pay_sales_invoices():
    """
    Automatically creates and submits Payment Entries for outstanding Sales Invoices
    for all customers belonging to the company 'Optilens BISKRA'.
    
    Returns:
        dict: A summary of processed customers and their invoices.
    """
    try:
        ...
        if not customers:
            return {'message': 'No customers found for the specified company.'}

        invoices_collection = []

        for customer in customers:
            # Fetch outstanding Sales Invoices for the current customer
            sales_invoices = frappe.get_all(
                'Sales Invoice',
                filters={'customer': customer.name, 'outstanding_amount': ['>', 0]},
                fields=['name', 'outstanding_amount', 'company', 'customer'],
                limit_page_length= 25  # Process up to 4 invoices per customer
            )

            if not sales_invoices:
                continue  # Skip to the next customer if no invoices are found

            for invoice in sales_invoices:
                try:
                    # Collect processed invoice names
                    ...
                    # Link Payment Entry to the Sales Invoice
                    reference_row = payment_entry.append('references', {})
                    reference_row.reference_doctype = 'Sales Invoice'
                    reference_row.reference_name = invoice.name
                    reference_row.allocated_amount = invoice.outstanding_amount

                    # Set Paid To account
                    default_bank_account = frappe.db.get_value('Company', invoice.company, 'default_bank_account')
                    if not default_bank_account:
                        frappe.throw(f"No default bank account set for the company {invoice.company}. Please configure it.")

                    payment_entry.paid_to = default_bank_account
                    payment_entry.paid_to_account_currency = frappe.db.get_value('Account', default_bank_account, 'account_currency')

                    # Set exchange rates if necessary
                    if payment_entry.company_currency != payment_entry.paid_to_account_currency:
                        payment_entry.target_exchange_rate = 1  # Adjust or fetch the correct rate dynamically
                        payment_entry.source_exchange_rate = 1

                    # Add reference details
                    payment_entry.reference_no = "AUTO-GEN"
                    ...
                    # Log and continue processing other invoices
                    frappe.log_error(f"Error processing invoice {invoice.name}: {str(e)}", "pay_sales_invoices")
                    continue
        ...
        frappe.log_error(f"Error in pay_sales_invoices: {str(e)}", "pay_sales_invoices")
        return {'error': str(e)}
